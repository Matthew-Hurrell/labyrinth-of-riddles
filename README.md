...
The Labyrinth of Riddles is a terminal game that a user can interact with via the Code Institute mock terminal on Heroku.
...
Navigating the game should be easy and intuitive for all users. The puzzle content should be challenging but well-balanced for a diverse audience. The text should be well displayed and easy to read. The narrative should be coherent and engaging to encourage users to continue to progress through the game.
...
There are two possible endings at the end of the program after the end function is called. These endings are defined using the functions of take_treasure and leave. Both of these functions call their respective storyboard functions and pass in the username variable, and then call the play_again function to give the user the option to play again or quit the game/program. Multiple endings were added to enhance game replayability and the user's overall experience. The endings also round off the narrative nicely and provide a good moral to the story.
...
Below is a basic data model flow chart showcasing how the underlying functions of the program fit together, and how user data is processed during the primary gameplay loops. The flow chart was created and used in the early planning stages of the program to plan the structure of the application. 
...
The Labyrinth of Riddles program has been tested extensively to check for user input validation errors. All inputs have been manually tested for uppercase, lowercase and surrounding whitespace characters with no bugs or errors detected. All user data inputs are processed and evaluated logically and effectively. Invalid inputs are detected and processed accordingly for every user input. The process name function has been tested with a mix of capitalization, input less than one character, input over twenty characters, surrounding whitespace and whitespace between words, and symbol/number characters. All errors were processed and evaluated effectively. 

The table below shows all the manual tests that were carried out on the Labyrinth of Riddles. The table was used as a visual guide to help methodically and systematically work through all the user inputs during the testing phase.

![Manual Input Validation Tests](readme-images/manual-testing.png)
...
The Labyrinth of Riddles is the third project submission for the [Code Institute](https://codeinstitute.net/) Higher National Diploma of Full Stack Software Development. The project focusses purely on back-end functionality with the Python programming language. 

I thoroughly enjoyed learning how to use Python to implement back-end data input processing into a basic program. I am very much looking forward to being able to tie together all the languages I have learnt in the previous modules to build a full-stack application for my next project. 
